...
class CardController extends Controller
{
                ...
                return DataTables::of($businessCards)
                    ...
                    ->editColumn('type', function ($card) {
                        if ($card->type === 'business') {
                            ...
                            return '<span class="badge bg-secondary text-white text-uppercase">' . trans($card->type) . '</span>';
                        }
                    })
    ...
    public function searchTheme(Request $request)
    {
        ...
        if ($cardType == "business") {
            $cards = Theme::where('theme_name', 'LIKE', '%' . $query . '%')->whereNotIn('theme_id', ["588969111014", "588969111015", "588969111016", "588969111017", "588969111018", "588969111019", "588969111020", "588969111021"])->where('theme_description', $type)->where('status', 1)->orderBy('id', 'desc')->get();
        } else {
            $cards = Theme::where('theme_name', 'LIKE', '%' . $query . '%')->whereIn('theme_id', ["588969111014", "588969111015", "588969111016", "588969111017", "588969111018", "588969111019", "588969111020", "588969111021"])->where('theme_description', $type)->where('status', 1)->orderBy('id', 'desc')->get();
        }

        // Translate theme names before returning the response
        $cards = $cards->map(function ($theme) {
            $theme->theme_name = __($theme->theme_name);
            return $theme;
        });
