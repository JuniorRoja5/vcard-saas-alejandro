                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-6 col-xl-6">
                                            ...
                                            <div class="mb-3" id="coverChooser">
                                                ...
                                                <!-- Campo oculto para Unsplash URL -->
...
<div class="modal modal-blur fade mt-10" id="themeModal" tabindex="-1" role="dialog" aria-hidden="true">
                                ...
                                <span class="text-center font-weight-bold">
                                    <img src="{{ asset('img/vCards/'.$theme->theme_thumbnail) }}"
                                        ...
                                        <div class="text-center">
                                            ...
                                            @php
                                            $vidSupportedIds = ["588969111146", "588969111145", "588969111144", "588969111143", "588969111142","588969111141", "588969111140", "588969111139", "588969111138","588969111137", "588969111136", "588969111135", "588969111134","588969111133", "588969111132", "588969111131", "588969111130","588969111129", "588969111128", "588969111127", "588969111126","588969111125","588969111124", "588969111123", "588969111122", "588969111121","588969111120", "588969111119", "588969111111", "588969111118","588969111117", "588969111116", "588969111115", "588969111114","588969111113", "588969111112", "588969111111", "588969111110", "588969111109", "588969111108", "588969111107","588969111106", "588969111105", "588969111104", "588969111103", "588969111102", "588969111101", "588969111100", "588969111099", "588969111098", "588969111097", "588969111096", "588969111094","588969111095", "588969111093", "588969111092", "588969111091", "588969111090", "588969111089", "588969111088", "588969111087", "588969111086"];
                                            @endphp
...
$(document).ready(() => {
    ...
    $("#coverType").change( function (){
        ...
        if(this.value == "photo"){
            ...
            // $("#cover").prop("required", true); // Comentado: validación custom más abajo
            // $("#logoNone").removeClass('d-none');
...
$('#searchInput').on('keyup', function() {
    ...
    let query = $(this).val();
    let type = 'vCard';
    let cardType = "{{ Request::get('type') ? Request::get('type') : 'business' }}";

    $.ajax({
        url: '{{ route('user.search.theme') }}',
        type: 'GET',
        data: {
            query: query, type: type, cardType: cardType
        },
        dataType: 'json',
        success: function(response) {
            let output = '';
...
<script>
// ClickMyLink - Unsplash Integration Fixed
function openUnsplashModal(button, type) {
    // Encontrar el input correcto basado en el botón
    const inputGroup = button.closest('.input-group') || button.closest('.mb-3');
    const targetInput = inputGroup.querySelector('input[type="text"]');
    
    if (!targetInput) {
        alert('Error: No se encontró el campo de imagen');
        return;
    }
    
    const placeholders = {
        'service': 'Services: consulting, coaching, therapy, meeting',
        'product': 'Products: business, course, digital, ebook'
    };
    
    showUnsplashModal(targetInput, placeholders[type] || 'Professional images');

// Validación personalizada para Cover (file OR Unsplash)
function validateCoverField() {
    const coverFile = document.getElementById("cover");
    const coverUnsplash = document.querySelector("input[name='cover']");
    
    // Si hay archivo local O URL de Unsplash, es válido
    const hasFile = coverFile && coverFile.files && coverFile.files.length > 0;
    const hasUnsplash = coverUnsplash && coverUnsplash.value && coverUnsplash.value.trim() !== "";
    
    return hasFile || hasUnsplash;
}
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (form) {
        form.addEventListener("submit", function(e) {
            ...
            // Solo validar si Cover Type es "photo"
            if (coverType && coverType.value === "photo") {
                if (!validateCoverField()) {
                    e.preventDefault();
                    alert("Por favor selecciona una imagen de Cover (archivo local o Unsplash)");
                    return false;
                }
            }
        });
...
function openCoverUnsplashModal() {
    ...
    if (!coverInput) {
        alert('Error: No se encontró el campo de cover');
        return;
    }
    showUnsplashModal(coverInput, 'Cover images: business, professional, background');
...
function showUnsplashModal(targetInput, placeholder) {
    // Remover modal existente
    const existingModal = document.getElementById('unsplashModal');
    if (existingModal) existingModal.remove();
    
    // Crear modal
    const modalHtml = `
        <div class="modal fade" id="unsplashModal" tabindex="-1" data-bs-backdrop="static">
            <div class="modal-dialog modal-xl">
                <div class="modal-content">
                    <div class="modal-header bg-primary text-white">
                        <h5 class="modal-title">
                            <i class="fas fa-images me-2"></i>Professional Images from Unsplash
                        </h5>
                    ...
                    <div class="modal-body">
                        <div class="row mb-3">
                            <div class="col-md-8">
                                <input type="text" class="form-control form-control-lg" id="unsplashQuery" 
                                       placeholder="${placeholder}" autocomplete="off">
                            </div>
                            <div class="col-md-4">
                                <button class="btn btn-primary btn-lg w-100" id="searchUnsplashBtn">
                                    <i class="fas fa-search me-2"></i>Search Images
                                </button>
                            </div>
                        </div>
                        
                        <div id="unsplashLoading" style="display: none;" class="text-center py-5">
                            <div class="spinner-border text-primary" style="width: 3rem; height: 3rem;"></div>
                            <p class="mt-3 text-muted">Searching professional images...</p>
                        </div>
                        
                        <div id="unsplashResults" class="row g-3" style="max-height: 60vh; overflow-y: auto;"></div>
                    </div>
                </div>
            </div>
        </div>`;
    ...
    // Focus en input
    setTimeout(() => document.getElementById('unsplashQuery').focus(), 500);
    
    // Event listeners
    document.getElementById('searchUnsplashBtn').addEventListener('click', () => {
        searchUnsplashImages(targetInput);
    });
    
    document.getElementById('unsplashQuery').addEventListener('keypress', (e) => {
        if (e.key === 'Enter') {
            e.preventDefault();
            searchUnsplashImages(targetInput);
        }
    ...
    // Búsqueda automática
    const defaultSearch = placeholder.includes(':') ? placeholder.split(':')[1].split(',')[0].trim() : 'business';
    document.getElementById('unsplashQuery').value = defaultSearch;
    setTimeout(() => searchUnsplashImages(targetInput), 1000);
...
function searchUnsplashImages(targetInput) {
            ...
            if (data.images && data.images.length > 0) {
                ...
                data.images.forEach(img => {
                    html += `<div class="col-lg-3 col-md-4 col-6">
                                <div class="unsplash-image card h-100" data-url="${img.regular}" 
                                     style="cursor: pointer; transition: all 0.3s; border: 2px solid transparent;">
                                    <img src="${img.small}" alt="${img.alt}" class="card-img-top" 
                                         style="height: 200px; object-fit: cover;">
                                    <div class="card-body p-2">
                                        <small class="text-muted d-block">
                                            <i class="fas fa-camera me-1"></i>by ${img.photographer}
                                        </small>
                                    </div>
                                </div>
                            </div>`;
