...
@section('css')
    <style>
        ...
        .badge {
            font-size: var(--tblr-badge-font-size) !important;
            padding: var(--tblr-badge-padding-y) var(--tblr-badge-padding-x) !important;
            border-radius: var(--tblr-badge-border-radius) !important;
        }
                        ...
                        <div class="card-body space-y-3">
                            ...
                            @if (isset($active_plan))
                                ...
                                @else
                                    ...
                                    @else
                                        <p class="fw-bold display-6">{{ $remaining_days > 0 ? floor($remaining_days) : __('Plan Expired!') }}
                                        </p>
                                        <span class="fw-bold">
                                            {{ __('Expires on') }}:
                                ...
                                <div class="card card-md">
                                    ...
                                    <div class="card-body">
                                        ...
                                        @if (Auth::user()->trial == 0 && $plan->trial != 0)
                                            <span class="badge bg-dark text-white text-uppercase">{{ __($plan->trial) }} {{ __('Days Free Trial') }}</span>
                                        @endif
                                        ...
                                        <div class="my-3">
                                            ...
                                            <small class="text-capitalize">
                                                ...
                                                @if ((int)$plan->plan_price != 0 && $plan->validity >= 365 && $plan->validity <= 366)
                                                    {{ __('Per') }} {{ __('Year') }}</span>
                                                @endif
                                                @if (str_contains(strtolower($plan->plan_name), 'trial') && (int)$plan->plan_price != 0 && $plan->validity > 1 && $plan->validity != 9999 && $plan->validity != 29 && $plan->validity != 30 && $plan->validity != 31 && $plan->validity != 365 && $plan->validity != 366 && $plan->validity != 9999)
                                        ...
                                        <ul class="list-unstyled lh-lg">
                                            ...
                                            @if ($plan->plan_type == 'BOTH' || $plan->plan_type == 'STORE')
                                                ...
                                                <li>
                                                    <i class="ti ti-{{ __($plan->no_of_categories > 0 ? 'check text-green' : 'x text-red') }} me-1"></i>
                                                    <strong>{{ $plan->no_of_categories == '999' ? __('Unlimited') : ($plan->no_of_categories != 0 ? $plan->no_of_categories : "") }} {{ __('Categories') }}</strong>
                                                </li>
                                                <li>
                                                    <i class="ti ti-{{ __($plan->no_of_store_products > 0 ? 'check text-green' : 'x text-red') }} me-1"></i>
                                                    <strong>{{ $plan->no_of_store_products == '999' ? __('Unlimited') : ($plan->no_of_store_products != 0 ? $plan->no_of_store_products : "") }} {{ __('Products') }}</strong>
                                                </li>
                                            @endif
                                            ...
                                            <li>
                                                <i class="ti ti-{{ __($plan->custom_domain == 1 ? 'check' : 'x') }} text-{{ $plan->custom_domain == 1 ? 'green' : 'red' }}"></i>
                                                <strong>{{ __('Custom Domain') }}</strong>
                                                <span class="badge badge-sm bg-green text-white text-uppercase ms-auto">{{ __('New') }}</span>
                                            </li>

                                            @if($config[76]->config_value == '1')
                                            <li>
                                                <i class="ti ti-{{ __($plan->nfc_card == 1 ? 'check' : 'x') }} text-{{ $plan->nfc_card == 1 ? 'green' : 'red' }}"></i>
                                                <strong>{{ __('Order NFC Card') }}</strong>
                                                <span class="badge badge-sm bg-green text-white text-uppercase ms-auto">{{ __('New') }}</span>
                                            </li>
                                            @endif
                                            <li>
                                                <i class="ti ti-{{ __($plan->storage > 0 ? 'check' : 'x') }} text-{{ $plan->storage > 0 ? 'green' : 'red' }}"></i>
                                                <strong>{{ __($plan->storage == '999' ? 'Unlimited' : ($plan->storage != 0 ? $plan->storage . "MB" : "")) }} {{ __('storage limit') }}</strong>
                                                <span class="badge badge-sm bg-green text-white text-uppercase ms-auto">{{ __('New') }}</span>
                                            <li><i
