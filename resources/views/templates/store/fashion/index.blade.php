...
<head>
    ...
    <style>
        ...
        .card,
        input,
        select, .rounded-modern, textarea, .alert, .badge {
...
<body dir="{{ App::isLocale('ar') || App::isLocale('ur') || App::isLocale('he') ? 'rtl' : 'ltr' }}">
    ...
    {{-- Page --}}
    <div id="wrapper" class="page m-0 min-h-screen" style="background-color: #F2F2F2;">
        ...
        <div class="page-wrapper m-0 p-0">
            <div class="page-body m-0 p-0">
                <div class="container-xl card rounded-0 p-0 overflow-hidden" style="min-height: 100vh;">
                    <!-- Navbar -->
                    @include('templates.store.fashion.includes.header', [
                        'bg_color' => 'dark',
                        'badge_color' => 'danger',
                    ])
                    ...
                    <div class="row mt-7 px-3 px-lg-4">
                        {{-- Greeting message --}}
                        <div class="col-md-12">
                            <h3 class="alert alert-important alert-dark p-3">{{ $business_card_details->sub_title }}
                            </h3>
                        </div>
                            ...
                            <swiper-container space-between="20" class="mySwiper" autoplay-delay="2500"
                                ...
                                @if (is_array(json_decode($business_card_details->cover)) == true)
                                    @foreach (json_decode($business_card_details->cover) as $cover)
                                        <swiper-slide>
                                            <img class="rounded-modern w-100 h-100 object-fit-cover"
                        ...
                        <div class="row {{ request('category') ? 'ps-2 ps-lg-3' : '' }}">
                            <!-- Categories -->
                            <div
                                class="{{ request('category') ? 'bg-opacity-dark rounded-modern col-md-3 my-3 px-3' : 'col-md-12 my-3' }}">
                                {{-- Filter --}}
                                ...
                                <h2
                                    class="head pt-4 pb-5 text-center position-relative fs-custom {{ request('category') ? 'd-none' : '' }}">
                                    {{ __('Categories') }}
                                    <div
                                        class="position-absolute start-50 translate-middle-x bg-dark bottom-bar-custom">
                                    </div>
                                </h2>

                                <h2
                                    class="head pt-4 pb-5 text-start position-relative fs-custom {{ request('category') ? '' : 'd-none' }}">
                                    {{ __('Categories') }}
                                    <div class="position-absolute start-0 bg-dark bottom-bar2"></div>
                                </h2>
                                                    ...
                                                    <div
                                                        ...
                                                        <div class="ratio ratio-1x1">
                                                            <img src="{{ url($category->thumbnail) }}"
                                                                class="object-cover rounded-modern w-100"
                            ...
                            <div class="mt-3 {{ request('category') ? 'col-md-9 ps-lg-4' : 'col-md-12' }}">
                                <div class="row align-items-center {{ request('category') ? '' : 'd-none' }}">
                                    ...
                                    <form class="row col-md-8" method="GET" action="{{ url()->current() }}">
                                        ...
                                        <div class="col-3">
                                            <button type="submit"
                                                class="btn btn-dark w-100">{{ __('Search') }}</button>
                                        </div>
                                    </form>
                                    ...
                                    <div class="row">
                                                                ...
                                                                <div class="carousel-inner">
                                                                    @foreach ($productImages as $productImage)
                                                                        <div
                                                                            ...
                                                                            <img class="d-block img-fluid object-fit-cover w-100 rounded-modern"
                                                        ...
                                                        <div class="">
                                                            ...
                                                            @if ($product->product_status == 'instock')
                                                                <a onclick="addToCart('{{ $product->product_id }}')"
                                                                    class="cursor-pointer btn btn-dark btn-lg w-100 rounded-modern fs-3 fw-bold btn-effect">
                                        ...
                                        <div class="col-md-12 my-3">
                                            @if (request()->has('category'))
                                                {{ $products->appends(request()->except('page'))->links('vendor.pagination.store', ['color' => 'dark']) }}
                                            @else
                                                {{ $products->links('vendor.pagination.store', ['color' => 'dark']) }}
                                            @endif
                                        </div>
                                    </div>
                                @else
                                        ...
                                        <ul class="list-inline list-inline-dots mb-0">
                                            <li class="list-inline-item"><a href="{{ $shareComponent['facebook'] }}"
                                                    target="_blank" class="link-light"><i
                                                        class="ti ti-brand-facebook-filled text-dark"></i></a></li>
                                            <li class="list-inline-item"><a href="{{ $shareComponent['twitter'] }}"
                                                    class="link-light"><i
                                                        class="ti ti-brand-twitter-filled text-dark"></i></a>
                                            </li>
                                            <li class="list-inline-item"><a href="{{ $shareComponent['linkedin'] }}"
                                                    target="_blank" class="link-light"><i
                                                        class="ti ti-brand-linkedin text-dark"></i></a></li>
                                            <li class="list-inline-item"><a href="{{ $shareComponent['telegram'] }}"
                                                    target="_blank" class="link-light"><i
                                                        class="ti ti-brand-telegram text-dark"></i></a></li>
                                            <li class="list-inline-item"><a href="{{ $shareComponent['whatsapp'] }}"
                                                    target="_blank" class="link-light"><i
                                                        class="ti ti-brand-whatsapp text-dark"></i></a></li>
                                        </ul>
                                    </div>
                                    <!-- Copyright -->
                                        ...
                                        <ul class="list-inline list-inline-dots mb-0">
                                            @if ($plan_details['hide_branding'] == 1)
                                                <li class="list-inline-item">
                                                    {{ __('Copyright') }} &copy; <span id="year"></span> <a
                                                        ...
                                                        class="link-light text-dark"><strong>{{ $card_details->title }}</strong></a>.
                                            ...
                                            @else
                                                <li class="list-inline-item">
                                                    {{ __('Copyright') }} &copy; <span id="year"></span> <a
                                                        href="{{ url($business_card_details->card_url) }}"
                                                        class="link-light text-dark"><strong>{{ config('app.name') }}</strong></a>.
                                                    {{ __('All rights reserved.') }}
                                                </li>
                                            @endif
                                        </ul>
        ...
        <!-- Bottom Navbar -->
        @include('templates.includes.bottom-bar', ['color' => 'blue', 'bg' => 'light'])
        <!-- End Bottom Navbar -->
    </div>
    ...
    <div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasEnd" aria-labelledby="offcanvasEndLabel">
        ...
        <div class="offcanvas-body">
            ...
            <div id="empty-cart" class="p-3">
                ...
                <!-- Start Shopping -->
                <a class="btn btn-dark d-flex btn-effect" data-bs-dismiss="offcanvas"
        ...
        <div class="offcanvas-footer border-top">
            ...
            <!-- Place Order -->
            <a class="btn btn-dark fs-2 btn-effect" id="place-order"
                        ...
                        <div class="modal-body">
                            <form action="{{ route('subscribe.store.newsletter') }}" method="post">
                                ...
                                <div class="row">
                                    ...
                                    <!-- Email -->
                                    <div class="col-8">
                                        <input type="email" class="form-control" name="email" placeholder="{{ __('Your email address') }}" required />
                                    </div>
                                    <!-- Button -->
                                    <div class="col-4">
                                        <button type="submit" class="btn btn-dark w-100">{{ __('Subscribe') }}</button>
                                    </div>
                                </div>
                            </form>
                        </div>
                        ...
                        <div class="modal-body">
                            ...
                            <div class="row d-flex justify-content-center align-items-center">
                                ...
                                <div class="w-25 h-25 mb-2">
                                    <img src="{{ url($img) }}" alt="{{ __($title) }}" class="img-fluid object-fit-cover rounded-4" />
                                </div>
                                {{-- Title --}}
                                <div class="mb-2">
                                ...
                                <div class="d-flex justify-content-center align-items-center">
                                    <a href="{{ $button_url }}" target="_blank" class="btn btn-dark w-100">{{ __($button_text) }}</a>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        ...
        function updateList() {
            ...
            cart.forEach((item, index) => {
                ...
                <div class="me-3 flex-shrink-0" style="width: 100px; height: 100px;">
                    <img src="${item.product_image}" class="img-fluid rounded-modern object-fit-cover h-100 w-100" alt="${item.product_name}" />
                </div>`;
            ...
            if (grandTotal > 0) {
                ...
                $("#cart-pricing").html(
                    `<h3 class="font-bold fs-2 d-flex justify-content-between"><span>{{ __('Grand total') }}</span><span class="text-dark">${jsFormatCurrency(grandTotal, setDecimalsPlaces, formatType)}</span></h3>`
                );
            } else {
                $("#cart-pricing").html(``);
            }
        ...
        function updateBadge() {
            ...
            if (badgeCount > 0) {
                ...
                $("#place-order").show().attr("class", "btn btn-dark d-flex");
            } else {
                $("#place-order").hide().attr("class", "btn btn-dark d-none");
                $("#badge").hide();
                $("#empty-cart").show();
            }
        }
