...
<head>
    ...
    @php
        // decode the json
        $custom_styles = json_decode($business_card_details->custom_styles, true);
    ...
    <style>
        body {
            font-family: '{{ $custom_styles['font_family'] }}', sans-serif;
        }
...
<body class="bg-gray-50 min-h-screen"
    ...
    <div class="container max-w-2xl mx-auto relative">
        {{-- Background Image (only if image is selected) --}}
        @php
            $backgroundType = $custom_styles['background_type'] ?? '';
            $backgroundColor = $custom_styles['background_color'] ?? '#ffffff';
            $gradientFrom = $custom_styles['gradient_start'] ?? '#ffffff';
            $gradientTo = $custom_styles['gradient_end'] ?? '#ffffff';
            $backgroundImage = $custom_styles['image_url'] ?? '';

            // button datas
            $buttonBackgroundType = $custom_styles['button_background_type'] ?? 'single_color';
            $buttonBackgroundColor = $custom_styles['button_background_color'] ?? '#000000';
            $buttonGradientFrom = $custom_styles['button_gradient_start'] ?? '#000000';
            $buttonGradientTo = $custom_styles['button_gradient_end'] ?? '#000000';
        ...
        @if ($backgroundType == 'image' && !empty($backgroundImage))
            <div class="fixed inset-0 z-0 flex justify-center items-start">
                <img src="{{ asset($backgroundImage) }}" class="max-w-2xl w-full h-full object-cover"
        ...
        @if ($business_card_details->password == null || Session::get('password_protected') == true)
            {{-- Check business details --}}
            @if ($business_card_details != null)
                <div class="shadow-[0_0_4px_rgba(0,0,0,0.1)] overflow-hidden">
                    <!-- Start Cover Image Section -->
                    @if ($business_card_details->cover_type == 'none')
                        <div class="lg:h-80 h-60 relative z-20" id="profile">
                            {{-- Cover Image --}}
                    ...
                    @if ($business_card_details->cover_type == 'vimeo-ap')
                        <div class="relative w-full z-20" style="padding-top: 56.25%;" id="profile">
                            {{-- Cover Video --}}
                            <iframe
                    ...
                    <!-- Profile Info -->
                    <div
                        class="px-4 -mt-0.5 pb-32 lg:p-8 relative min-h-screen overflow-y-auto shadow-[0_0_4px_rgba(0,0,0,0.1)]-xl
                            @if ($backgroundType == 'single_color') bg-[{{ $backgroundColor }}]
                            @elseif($backgroundType == 'gradient') bg-gradient-to-r from-[{{ $gradientFrom }}] to-[{{ $gradientTo }}] @endif">
                        <!-- Start Profile Info -->

                        {{-- Row Layout --}}
                        @if ($custom_styles['layout'] == 'row')
                            <div class="text-start mt-4 relative flex z-20">
                                {{-- Profile Image --}}

                                <img src="{{ url($business_card_details->profile) }}"
                                    alt="{{ $business_card_details->title }}"
                                    class="w-32 h-32 bg-transparent object-cover {{ $custom_styles['profile_image_style'] == 'circle'
                                        ? 'rounded-full'
                                ...
                                <div class="relative flex flex-col ml-6">
                                    ...
                                    @if (isset($business_card_details->description) || isset($business_card_details->address))
                                        <div
                                            class="mt-3 text-sm text-[{{ $custom_styles['description_color'] }}] leading-relaxed font-medium tracking-tight line-clamp-3">
                        ...
                        @elseif ($custom_styles['layout'] == 'column')
                            <div class="text-center mt-4 relative flex flex-col justify-center items-center z-20">
                                {{-- Profile Image --}}
                                ...
                                <div class="relative flex flex-col">
                                    ...
                                    @if (isset($business_card_details->description) || isset($business_card_details->address))
                                        <div
                                            class="mt-3 text-base text-[{{ $custom_styles['description_color'] }}] leading-relaxed font-medium tracking-tight text-center line-clamp-3">
                                            {!! $business_card_details->description !!}
                                        </div>
                                    @endif
                                </div>
                                    ...
                                    @if (in_array($feature->type, ['tel', 'address', 'wa', 'instagram']))
                                        ...
                                        @if ($feature->type == 'address')
                                            <a href="#location"
                                                ...
                                                @if ($buttonEdge == 'rounded') rounded-2xl @endif focus:outline-none w-14 h-14 flex items-center justify-center">
                                                <i
                                                    class="{{ $feature->icon }} fa-xl text-[{{ $buttonIconColor }}]"></i>
                                            </a>
                                        @endif
                                        {{-- Phone --}}
                                        @if ($feature->type == 'tel')
                                    ...
                                    <div class="grid grid-cols-1 gap-4 relative">
                                        <div class="mt-8 w-full ">
                                            ...
                                            <a href="https://www.google.com/maps/place/{{ urlencode($feature->content) }}"
                                                ...
                                                class="@if ($buttonBackgroundType == 'single_color') bg-[{{ $buttonBackgroundColor }}] @endif
                                                @if ($buttonBackgroundType == 'gradient') bg-gradient-to-r from-[{{ $buttonGradientFrom }}] to-[{{ $buttonGradientTo }}] @endif
                                                @if ($buttonEdge == 'rounded') rounded-2xl @endif px-6 py-4 flex flex-col justify-center focus:outline-none">
                                                <!-- Font Awesome Icon -->
                                                <i
                                                    class="{{ $feature->icon }} fa-xl text-[{{ $buttonIconColor }}] text-2xl py-4"></i>
                                                <!-- Title -->
                                                <h2 class="text-[{{ $buttonTextColor }}] text-md font-semibold">
                                                    {{ $feature->label }}
                                                </h2>
                                                <!-- Description -->
                                                <p class="text-[{{ $buttonTextColor }}] text-sm flex items-center">
                                                    {{ $feature->content }}
                                                </p>
                                            </a>
                                        </div>
                                    ...
                                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                        @foreach ($validFeatures as $feature)
                                            ...
                                            <a href="{{ $href }}" target="_blank"
                                                ...
                                                <!-- Font Awesome Icon -->
                                                <i
                                                    class="{{ $feature->icon ?? '' }} text-[{{ $buttonIconColor }}] text-xl mb-1.5"></i>
                                                <!-- Title -->
                                                <h2 class="text-[{{ $buttonTextColor }}] text-md font-semibold">
                                                    {{ $feature->label ?? 'N/A' }}</h2>
                                                <!-- Description -->
                                                <p class="text-[{{ $buttonTextColor }}] text-sm truncate">
                                                    {{ $feature->content ?? '' }}
                                                </p>
                                            </a>
                                        @endforeach
                                    ...
                                    @foreach ($service_details as $service_detail)
                                        <swiper-slide class="p-0.5">
                                            ...
                                            <div
                                                ...
                                                @if ($custom_styles['card_edge'] == 'rounded') rounded-2xl @endif p-4 relative">
                                                {{-- Image --}}
                                                <img class="w-full h-44 object-cover mb-4 @if ($buttonEdge == 'rounded') rounded-2xl @endif"
                                                    src="{{ url($service_detail->service_image) }}"
                                                    alt="{{ $service_detail->service_name }}" />
                                                {{-- Name --}}
                                                <h2
                                                    class="text-[{{ $custom_styles['title_color'] }}] text-lg font-bold mb-2">
                                                    {{ $service_detail->service_name }}
                                                </h2>
                                                {{-- Description --}}
                                                <p
                                                    class="text-[{{ $custom_styles['description_color'] }}] font-normal mb-2">
                                                    ...
                                                    @if ($enquiry_button != null)
                                                        @if ($whatsAppNumberExists == true && $whatsAppNumberExists == true && $service_detail->enable_enquiry == 'Enabled')
                                                            <a href="https://wa.me/{{ $enquiry_button }}?text={{ __('Hi, I am interested in your product/service:') }} {{ $service_detail->service_name }}. {{ __('Please provide more details.') }}"
                                                                ...
                                                                class="@if ($buttonBackgroundType == 'single_color') bg-[{{ $buttonBackgroundColor }}] @endif
                                                                @if ($buttonBackgroundType == 'gradient') bg-gradient-to-r from-[{{ $buttonGradientFrom }}] to-[{{ $buttonGradientTo }}] @endif
                                                                @if ($buttonEdge == 'rounded') rounded-2xl @endif text-[{{ $buttonTextColor }}] w-full px-12 lg:w-auto text-base py-3 transition-colors block text-center">
                                                                {{ __('Enquire') }}
                                                            </a>
                                                        @endif
                                                    @endif
                                        ...
                                        <swiper-slide class="p-0.5">
                                            ...
                                            <div
                                                ...
                                                @if (!empty($product_detail->badge))
                                                    <p
                                                        class="absolute top-7 right-7 font-medium text-[{{ $buttonTextColor }}] @if ($buttonBackgroundType == 'single_color') bg-[{{ $buttonBackgroundColor }}] @endif
                                                                @if ($buttonBackgroundType == 'gradient') bg-gradient-to-r from-[{{ $buttonGradientFrom }}] to-[{{ $buttonGradientTo }}] @endif
                                                                @if ($buttonEdge == 'rounded') rounded-2xl @endif  px-4 py-1">
                                                        {{ $product_detail->badge }}
                                                    </p>
                                                @endif
                                                {{-- Image --}}
                                                <img class="w-full h-44 object-cover @if ($buttonEdge == 'rounded') rounded-2xl @endif mb-2"
                                                    src="{{ url($product_detail->product_image) }}"
                                                    alt="{{ $product_detail->product_name }}" />
                                                {{-- Name --}}
                                                <h2
                                                    class="text-[{{ $custom_styles['title_color'] }}] text-md font-semibold">
                                                    {{ $product_detail->product_name }}
                                                </h2>
                                                {{-- Description --}}
                                                <p
                                                    class="text-[{{ $custom_styles['description_color'] }}] text-sm mt-2 ">
                                                ...
                                                <div class="flex-flex-col mt-2">
                                                    ...
                                                    <div class="flex flex-col mb-4">
                                                        <div>
                                                            <h4
                                                                class="text-[{{ $custom_styles['title_color'] }}] text-md font-bold">
                                                                {{ __('Price:') }}
                                                                <span class="text-gray-500 font-bold text-base ml-1">
                                                    ...
                                                    @if ($enquiry_button != null)
                                                        @if ($whatsAppNumberExists == true)
                                                            <a href="https://wa.me/{{ $enquiry_button }}?text={{ __('Hi, I am interested in your product:') }} {{ $product_detail->product_name }}. {{ __('Please provide more details.') }}"
                                                                ...
                                                                class="@if ($buttonBackgroundType == 'single_color') bg-[{{ $buttonBackgroundColor }}] @endif
                                                                @if ($buttonBackgroundType == 'gradient') bg-gradient-to-r from-[{{ $buttonGradientFrom }}] to-[{{ $buttonGradientTo }}] @endif
                                                                @if ($buttonEdge == 'rounded') rounded-2xl @endif text-[{{ $buttonTextColor }}] font-semibold block text-center py-3 px-12">
                                                                {{ __('Enquire') }}
                                                            </a>
                                                        @endif
                                                    @endif
                                    ...
                                    <swiper-container slides-per-view="1" class="mySwiper" loop="true"
                                        ...
                                        @foreach ($galleries_details as $galleries_detail)
                                            <swiper-slide class="p-0.5">
                                                <!-- Gallery -->
                                                <div
                                                    class="flex flex-col items-center justify-center bg-white p-4 @if ($custom_styles['card_edge'] == 'rounded') rounded-2xl @endif">
                                                    @if ($galleries_detail->caption)
                                                        <h3
                                                            class="py-3 @if ($buttonBackgroundType == 'single_color') bg-[{{ $buttonBackgroundColor }}] @endif
                                                                @if ($buttonBackgroundType == 'gradient') bg-gradient-to-r from-[{{ $buttonGradientFrom }}] to-[{{ $buttonGradientTo }}] @endif
                                                                @if ($buttonEdge == 'rounded') rounded-2xl @endif text-[{{ $buttonTextColor }}] mb-3 text-lg w-full text-center">
                                                            {{ $galleries_detail->caption }}</h3>
                                                    @endif
                                                    <img class="w-full h-64 object-cover @if ($buttonEdge == 'rounded') rounded-2xl @endif"
                                ...
                                <div class="grid sm:grid-cols-2 lg:grid-cols-2 gap-4 items-center">
                                    ...
                                    @foreach ($feature_details as $feature)
                                        @if ($feature->type == 'youtube')
                                            <!-- Video 1 -->
                                            <div
                                                class="overflow-hidden @if ($custom_styles['card_edge'] == 'rounded') rounded-2xl @endif">
                                                {{-- Add Youtube title --}}
                                ...
                                <div class="grid grid-cols-1 gap-4 items-center">
                                    ...
                                    @foreach ($feature_details as $feature)
                                        @if ($feature->type == 'iframe')
                                            <div
                                                class="overflow-hidden @if ($custom_styles['card_edge'] == 'rounded') rounded-t-2xl @endif">
                                                ...
                                                @if ($feature->label != null)
                                                    <div
                                                        class="px-5 py-3 @if ($buttonBackgroundType == 'single_color') bg-[{{ $buttonBackgroundColor }}] @endif
                                                                @if ($buttonBackgroundType == 'gradient') bg-gradient-to-r from-[{{ $buttonGradientFrom }}] to-[{{ $buttonGradientTo }}] @endif
                                                                @if ($custom_styles['card_edge'] == 'rounded') rounded-b-2xl @endif text-[{{ $buttonTextColor }}]">
                            ...
                            <div class="relative">
                                ...
                                @if ($plan_details['appointment'] == 1)
                                    @if ($appointment_slots != null)
                                        ...
                                        <div
                                            class="overflow-hidden p-10 bg-white @if ($custom_styles['card_edge'] == 'rounded') rounded-2xl @endif">
                                            ...
                                            <div
                                                ...
                                                <input type="text" id="appointment-date"
                                                    class="flatpickr-input md:w-1/2 @if ($buttonEdge == 'rounded') rounded-xl @endif w-full px-4 py-3 text-[#121212] bg-white border border-gray-300 focus:outline-none"
                                                    placeholder="{{ __('Select a date') }}" required />
                                                <!-- Select time in dropdown -->
                                                <select id="time-slot-select" required
                                                    class="md:w-1/2 w-full px-4 py-3 @if ($buttonEdge == 'rounded') rounded-xl @endif text-[#121212] bg-white border border-gray-300 focus:outline-none">
                                            ...
                                            <div class="flex justify-center">
                                                <button id="add-slot-button"
                                                    class="w-full p-3 @if ($buttonBackgroundType == 'single_color') bg-[{{ $buttonBackgroundColor }}] @endif
                                                                @if ($buttonBackgroundType == 'gradient') bg-gradient-to-r from-[{{ $buttonGradientFrom }}] to-[{{ $buttonGradientTo }}] @endif
                                                                @if ($buttonEdge == 'rounded') rounded-2xl @endif text-[{{ $buttonTextColor }}] text-lg text-center font-semibold focus:outline-none"
                            ...
                            <div class="relative">
                                ...
                                <div class="grid lg:grid-cols-2 gap-4">
                                    ...
                                    @foreach ($payment_details as $payment)
                                        ...
                                        <div
                                            class="flex flex-col p-4 @if ($buttonBackgroundType == 'single_color') bg-[{{ $buttonBackgroundColor }}] @endif
                                            @if ($buttonBackgroundType == 'gradient') bg-gradient-to-r from-[{{ $buttonGradientFrom }}] to-[{{ $buttonGradientTo }}] @endif
                                            @if ($buttonEdge == 'rounded') rounded-2xl @endif">
                                            <div class="flex justify-between items-center">
                                                {{-- Payment icon/image --}}
                                                @include('templates.partials.payment-link-image', [
                                                    'iconColor' => 'text-[' . $buttonIconColor . ']',
                                                ])
                                                ...
                                                @if ($payment->type == 'url')
                                                    <a href="https://{{ str_replace('https://', '', $payment->content) }}"
                                                        ...
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="24"
                                                            ...
                                                            class="icon icon-tabler icons-tabler-outline icon-tabler-external-link text-[{{ $buttonIconColor }}] h-8 w-8">
                                            ...
                                            <h3
                                                class="font-medium text-[{{ $buttonTextColor }}] {{ $payment->type == 'text' ? 'py-3' : 'pt-3' }}">
                                                {{ $payment->label }}</h3>
                                            <!-- Bank Details (Optional) -->
                                            @if ($payment->type == 'text')
                                                <p
                                                    class="text-[{{ $buttonTextColor }}] text-sm break-word text-base">
                                    ...
                                    <div class="bg-white p-4 @if ($custom_styles['card_edge'] == 'rounded') rounded-2xl @endif">
                                            ...
                                            <div class="grid grid-cols-2 lg:grid-cols-3 gap-6">
                                                @foreach (['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'] as $day)
                                                    <div class="flex items-center space-x-4">
                                                        <!-- Day Icon -->
                                                        <div
                                                            class="flex items-center justify-center w-10 h-10 @if ($buttonBackgroundType == 'single_color') bg-[{{ $buttonBackgroundColor }}] @endif
                                                                @if ($buttonBackgroundType == 'gradient') bg-gradient-to-r from-[{{ $buttonGradientFrom }}] to-[{{ $buttonGradientTo }}] @endif
                                                                @if ($buttonEdge == 'rounded') rounded-2xl @endif text-[{{ $buttonTextColor }}]">
                                                            <svg xmlns="http://www.w3.org/2000/svg" width="24"
                                                                height="24" viewBox="0 0 24 24" fill="none"
                                                                stroke="currentColor" stroke-width="2"
                                                                stroke-linecap="round" stroke-linejoin="round"
                                                        ...
                                                        <div>
                                                            <p
                                                                class="text-sm font-semibold text-[{{ $custom_styles['title_color'] }}] capitalize">
                                                                {{ __($day) }}</p>
                                                            <p
                                                                class="text-base text-[{{ $custom_styles['sub_title_color'] }}]">
                                        ...
                                        @else
                                            ...
                                            <div class="flex items-start space-x-4">
                                                <!-- Animated Icon -->
                                                <div
                                                    class="flex items-center justify-center w-12 h-12 bg-gray-200 text-[{{ $buttonIconColor }}] rounded-full transform hover:scale-110 transition-transform duration-300 ease-in-out">
                                ...
                                <swiper-container class="mySwiper" autoplay-delay="2000"
                                    ...
                                    @foreach ($testimonials as $testimonial)
                                        ...
                                        <swiper-slide class="p-1 rounded-2xl bg-transparent">
                                            <div
                                                class="mb-2 flex flex-col items-center justify-center p-6 text-center bg-white @if ($custom_styles['card_edge'] == 'rounded') rounded-2xl @endif">
                                                {{-- Review --}}
                                                <p
                                                    class="text-[{{ $custom_styles['sub_title_color'] }}] text-lg w-full mt-2">
                                                    "{{ $testimonial->review }}"
                                                </p>
                                                {{-- Image --}}
                                                ...
                                                <div>
                                                    {{-- Name --}}
                                                    <h4
                                                        class="text-[{{ $custom_styles['title_color'] }}] font-bold text-center ">
                                                        {{ $testimonial->reviewer_name }}
                                                    </h4>
                                                    {{-- Position --}}
                                                    @if ($testimonial->review_subtext)
                                                        <p
                                                            class="text-[{{ $custom_styles['description_color'] }}] text-sm text-center ">
                                ...
                                @foreach ($feature_details as $feature)
                                    @if ($feature->type == 'map')
                                        <iframe src="https://www.google.com/maps/embed?{!! $feature->content !!}"
                                            ...
                                            loading="lazy"
                                            class="@if ($custom_styles['card_edge'] == 'rounded') rounded-t-2xl @endif"></iframe>
                                        {{-- Map title --}}
                                        @if ($feature->label != null)
                                            <div
                                                class="px-5 py-3 @if ($buttonBackgroundType == 'single_color') bg-[{{ $buttonBackgroundColor }}] @endif
                            ...
                            @if ($business_card_details->enquiry_email != null)
                                <div class="relative">
                                    ...
                                    @if (Session::has('message'))
                                        <div
                                            class="px-6 py-4 mb-6 @if ($buttonBackgroundType == 'single_color') bg-[{{ $buttonBackgroundColor }}] @endif
                                            @if ($buttonBackgroundType == 'gradient') bg-gradient-to-r from-[{{ $buttonGradientFrom }}] to-[{{ $buttonGradientTo }}] @endif
                                                @if ($buttonEdge == 'rounded') rounded-2xl @endif">
                                            <div class="flex items-start">
                                                <div class="mr-4">
                                                    <svg class="w-6 h-6 text-[{{ $buttonIconColor }}]"
                                                ...
                                                <div>
                                                    <p class="font-semibold text-[{{ $buttonTextColor }}]">
                                                        {{ Session::get('message') }}</p>
                                                    <p class="text-sm text-[{{ $buttonTextColor }}]">
                                    ...
                                    <form
                                        class="w-full max-w-full border px-6 pt-9 pb-6 bg-white @if ($custom_styles['card_edge'] == 'rounded') rounded-2xl @endif relative z-10"
                                        ...
                                        <div class="grid grid-cols-1 gap-2 ">
                                            ...
                                            <div class="h-full pb-4">
                                                ...
                                                <textarea name="message" placeholder="{{ __('Your Message') }}"
                                                    class="w-full px-4 py-2 border border-gray-300 rounded-xl focus:outline-none" required style="min-height: 10rem"></textarea>
                                            </div>
                                        ...
                                        <button type="submit"
                                            class="w-full px-4 py-4 text-xl font-semibold rounded-2xl focus:outline-none @if ($buttonBackgroundType == 'single_color') bg-[{{ $buttonBackgroundColor }}] @endif
                                                                @if ($buttonBackgroundType == 'gradient') bg-gradient-to-r from-[{{ $buttonGradientFrom }}] to-[{{ $buttonGradientTo }}] @endif
                                                                @if ($buttonEdge == 'rounded') rounded-2xl @endif text-[{{ $buttonTextColor }}]">
                                            {{ __('Send') }}
                                        </button>
                        ...
                        @else
                            <div class="pb-1">
                                <div
                                    class="flex px-3 m-auto pt-5 font-semibold text-sm flex-col md:flex-row max-w-6xl">
                                    <div class="mt-2 text-[{{ $custom_styles['sub_title_color'] }}]">
                                        {{ __('Made with') }}
                                        <a class="text-[{{ $custom_styles['title_color'] }}]"
                                            href="{{ env('APP_URL') }}">
            ...
            <div
                class="fixed w-11/12 left-1/2 bottom-4 bg-[{{ $custom_styles['bottom_bar_color'] }}]/60 z-20 rounded-full backdrop-blur-md py-4 px-3 flex lg:hidden md:hidden transform -translate-x-1/2">
                <!-- Profile Icon -->
                <div class="flex-1 flex items-center justify-center">
                    <a class="p-3 @if ($buttonBackgroundType == 'single_color') bg-[{{ $buttonBackgroundColor }}] @endif
                        @if ($buttonBackgroundType == 'gradient') bg-gradient-to-r from-[{{ $buttonGradientFrom }}] to-[{{ $buttonGradientTo }}] @endif
                        rounded-full" href="#profile">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                            fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                            stroke-linejoin="round"
                            class="icon icon-tabler icons-tabler-outline icon-tabler-user text-[{{ $buttonIconColor }}] h-6 w-6">
                            <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                ...
                <!-- Send Icon -->
                <div class="flex-1 flex items-center justify-center">
                    <button class="p-3 @if ($buttonBackgroundType == 'single_color') bg-[{{ $buttonBackgroundColor }}] @endif
                        @if ($buttonBackgroundType == 'gradient') bg-gradient-to-r from-[{{ $buttonGradientFrom }}] to-[{{ $buttonGradientTo }}] @endif
                        rounded-full"
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                            fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                            stroke-linejoin="round"
                            class="icon icon-tabler icons-tabler-outline icon-tabler-send text-[{{ $buttonIconColor }}] h-6 w-6">
                            <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                            <path d="M10 14l11 -11" />
                ...
                <div class="flex-1 flex items-center justify-center">
                    <a href="{{ route('download.vCard', $business_card_details->card_id) }}"
                        class="p-3 @if ($buttonBackgroundType == 'single_color') bg-[{{ $buttonBackgroundColor }}] @endif
                        @if ($buttonBackgroundType == 'gradient') bg-gradient-to-r from-[{{ $buttonGradientFrom }}] to-[{{ $buttonGradientTo }}] @endif
                        rounded-full">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                            fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                            stroke-linejoin="round"
                            class="icon icon-tabler icons-tabler-outline icon-tabler-download text-[{{ $buttonIconColor }}] h-6 w-6">
                            <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                ...
                <div class="flex-1 flex items-center justify-center">
                    <button class="p-3 @if ($buttonBackgroundType == 'single_color') bg-[{{ $buttonBackgroundColor }}] @endif
                        @if ($buttonBackgroundType == 'gradient') bg-gradient-to-r from-[{{ $buttonGradientFrom }}] to-[{{ $buttonGradientTo }}] @endif
                        rounded-full" onclick="toggleScanModal(true)">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                            fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                            stroke-linejoin="round"
                            class="icon icon-tabler icons-tabler-outline icon-tabler-line-scan text-[{{ $buttonIconColor }}] h-6 w-6">
                            <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                ...
                <div class="flex-1 flex items-center justify-center">
                    <button class="p-3 @if ($buttonBackgroundType == 'single_color') bg-[{{ $buttonBackgroundColor }}] @endif
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                            fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                            stroke-linejoin="round"
                            class="icon icon-tabler icons-tabler-outline icon-tabler-share text-[{{ $buttonIconColor }}] h-6 w-6">
                            <path stroke="none" d="M0 0h24v24H0z" fill="none" />
            ...
            <div class="bg-white rounded-2xl w-full max-w-md p-6 mx-4 shadow-lg">
                ...
                <form id="appointmentForm">
                    ...
                    <div class="flex justify-between">
                        <button type="button" class="bg-black text-white px-4 py-2 rounded-lg"
                            onclick="validateAndShowModal()">
                            {{ __('Close') }}
                        </button>
                        <button type="submit" id="bookAppointmentButton"
                            class="@if ($buttonBackgroundType == 'single_color') bg-[{{ $buttonBackgroundColor }}] @endif
                            @if ($buttonBackgroundType == 'gradient') bg-gradient-to-r from-[{{ $buttonGradientFrom }}] to-[{{ $buttonGradientTo }}] @endif
                                text-[{{ $buttonTextColor }}] text-white px-4 py-2 rounded-lg">
                            {{ __('Submit') }}
                        </button>
                    </div>
                </form>
            ...
            <div class="bg-white rounded-2xl w-full max-w-md p-6 mx-4 space-y-6" onclick="event.stopPropagation()">
                ...
                <div class="flex justify-center">
                    <button onclick="copyLink()"
                        class="@if ($buttonBackgroundType == 'single_color') bg-[{{ $buttonBackgroundColor }}] @endif
                        @if ($buttonBackgroundType == 'gradient') bg-gradient-to-r from-[{{ $buttonGradientFrom }}] to-[{{ $buttonGradientTo }}] @endif
                        @if ($buttonEdge == 'rounded') rounded-2xl @endif text-[{{ $buttonTextColor }}] font-bold py-2 px-4 w-full">
                        {{ __('Copy Link') }}
                    </button>
                </div>
            </div>
            ...
            <div class="rounded-2xl w-full max-w-md p-6 mx-4 space-y-6 bg-white" onclick="event.stopPropagation()">
                ...
                <div>
                    ...
                    <input type="text" id="whatsappNumber" placeholder="{{ __('e.g., +919876543210') }}"
                        class="w-full mt-1 px-4 py-3 border border-gray-300 rounded-2xl focus:outline-none" />
                </div>
                ...
                <div class="flex justify-center">
                    <button onclick="sendMessage()"
                        class="@if ($buttonBackgroundType == 'single_color') bg-[{{ $buttonBackgroundColor }}] @endif
                        @if ($buttonBackgroundType == 'gradient') bg-gradient-to-r from-[{{ $buttonGradientFrom }}] to-[{{ $buttonGradientTo }}] @endif
                        @if ($buttonEdge == 'rounded') rounded-2xl @endif text-[{{ $buttonTextColor }}] font-bold py-3 px-4 w-full focus:outline-none">
                        {{ __('Send') }}
                    </button>
                </div>
            </div>
                ...
                <div class="flex justify-center">
                    <button id="download"
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                            fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                            stroke-linejoin="round"
                            class="icon icon-tabler icons-tabler-outline icon-tabler-download h-6 w-6">
                            <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                            <path d="M4 17v2a2 2 0 0 0 2 2h12a2 2 0 0 0 2 -2v-2" />
                            <path d="M7 11l5 5l5 -5" />
                            <path d="M12 4l0 12" />
